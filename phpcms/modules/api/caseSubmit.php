...
class caseSubmit  extends api{
    ...
    public function caseSubmitData_update(){
        $case_id = $_POST['case_id'];
         $userid =  param::get_cookie('_userid');
        $_POST['anlitijiao']['mcs_userid'] = $userid;
        $_POST['anlitijiao']['mcs_id'] = $case_id ;
        //这个是分享者的照片
        $tmp_name =$_FILES['mcs_lecturerThumbs']['tmp_name'];  // 文件上传后得临时文件名
        $name     =$_FILES['mcs_lecturerThumbs']['name'];     // 被上传文件的名称
        $size     =$_FILES['mcs_lecturerThumbs']['size'];    //  被上传文件的大小
        ...
        $return = $this->curl->curl_action('user-api/case-submit-update', ['anlitijiao'=>$_POST['anlitijiao']]);
       showmessage('案例修改成功','http://localhost/index.php?m=api&c=myCaseList');
    }
