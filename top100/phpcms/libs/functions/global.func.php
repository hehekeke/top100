...
/**
	 ...
	 * @param int $viewed 真实的浏览次数
	 */
	function viewed_num($addtime,$viewed){
	    //访问次数（当前时间 - 发布时间）/  720  / 发布时间最后一位数字 *  发布时间第二为数字 + 真实浏览数量
	    $time = time();
	    
	    $last = intval(substr("".$addtime,-1));
	    $last = $lasrt > 0 ? $last : 1;
	    
	    $lasttwo = substr("".$addtime,-2,1);
	    $lasttwo = $lasttwo > 0 ? $lasttwo : 1;
	   // echo $addtime;exit;
	    $times = (($time - $addtime) / 720 /  $last ) + $viewed + substr("".$addtime,-3);
	    $times = intval($times);
	//     echo $times;exit;
	 ...
	 * @param int $viewed 真实的浏览次数
	 */
	function collected_num($addtime,$viewed,$num){
	    //访问次数（当前时间 - 发布时间）/  720  / 发布时间最后一位数字 *  发布时间第二为数字 + 真实浏览数量
	    $time = time();
	...
	//     echo $addtime.'<br />';
	    ...
	    $view = viewed_num($addtime, $viewed);
	    
	    $time = $view / 15 * 2 + $num + $last * 10;
	    $time = intval($time);
	//     echo $times;exit;
	 ...
	 * @param int $viewed 真实的浏览次数
	 */
	function join_num($addtime,$viewed,$num){
	    //访问次数（当前时间 - 发布时间）/  720  / 发布时间最后一位数字 *  发布时间第二为数字 + 真实浏览数量
	    $time = time();
